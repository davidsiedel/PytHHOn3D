...
orders = [0, 1, 2]
orders = [0, 1, 2]
for order_count, order in enumerate(orders):
    # ------------------------------------------------------------------------------------------------------------------
    ...
    stabilization_parameter = 1.0e1  # K2
    ...
    # cell_sol = lambda x, x_T, v_T, u: np.sum([u[i] * ((x - x_T) / v_T) ** i for i in range(order_count + 1)])
    cell_sol = lambda x, x_T, v_T, u: np.sum([u[i] * ((x - x_T) / v_T) ** i for i in range(order + 1)])
    # ------------------------------------------------------------------------------------------------------------------
    ...
    ax = axes[order_count]
    ax.plot(x_anal, y_anal, color="black", linewidth=1.0, dashes=[2, 2])
    # ------------------------------------------------------------------------------------------------------------------
    ...
    ax.set_ylabel("Solution, $l = {}, k = 1$".format(str(order_count)))
